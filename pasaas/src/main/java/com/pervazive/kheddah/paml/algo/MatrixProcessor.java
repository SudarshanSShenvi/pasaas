...
public final class MatrixProcessor {
  ...
  public static String toString(double[][] a) {
    ...
    sb.append("       ");
    for (int j = 0; j < cols; j++) {
      new Formatter(sb, Locale.US).format("     [%1$3d]", j);
    }
    sb.append(CR);

    for (int i = 0; i < rows; i++) {
      new Formatter(sb, Locale.US).format(" [%1$3d] ", i);
      for (int j = 0; j < cols; j++) {
        new Formatter(sb, Locale.US).format(" %1$ 6f", a[i][j]);
      }
      sb.append(CR);
    }
  ...
  public static String toCodeString(double[][] a) {
    ...
    for (int i = 0; i < rows; i++) {
      sb.append("{");
      for (int j = 0; j < cols; j++) {
        new Formatter(sb, Locale.US).format(" %1$ 4f", a[i][j]);
        sb.append(", ");
      }
